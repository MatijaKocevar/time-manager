-- Fix timezone: Remove forced UTC conversion to use server/local timezone for date grouping
-- This fixes entries showing on wrong date (e.g., 2026-01-20 00:24 local showing as 2026-01-19)
...
WITH date_range AS (
    ...
    SELECT DISTINCT
        "userId" AS user_id,
        DATE("startTime" AT TIME ZONE 'UTC' AT TIME ZONE 'Europe/Ljubljana') AS normalized_date,
        type
    FROM "TaskTimeEntry"
...
tracked_hours_base AS (
    ...
    WHERE "endTime" IS NOT NULL 
        AND duration IS NOT NULL
    GROUP BY "userId", DATE("startTime" AT TIME ZONE 'UTC' AT TIME ZONE 'Europe/Ljubljana'), type
),
