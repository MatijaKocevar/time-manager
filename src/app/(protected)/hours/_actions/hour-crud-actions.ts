...
import { TASK_ID_VALUES } from "../_constants/hour-types"
import {
...
import { mapRequestTypeToHourType } from "@/app/(protected)/shifts/_utils/request-shift-mapping"
import { getOrCreateSystemTaskForHourType } from "@/app/(protected)/tracker/_utils/system-task-helpers"

export async function createHourEntry(input: CreateHourEntryInput) {
    try {
        const session = await requireAuth()

        const validation = validateInput(CreateHourEntrySchema, input)
...
export async function updateHourEntry(input: UpdateHourEntryInput) {
    try {
        ...
        const validation = validateInput(UpdateHourEntrySchema, input)
        if (!validation.success) {
            return { error: validation.error }
        }
...
export async function deleteHourEntry(input: DeleteHourEntryInput) {
    try {
        ...
        const validation = validateInput(DeleteHourEntrySchema, input)
        if (!validation.success) {
            return { error: validation.error }
        }
...
export async function bulkCreateHourEntries(input: BulkCreateHourEntriesInput) {
    try {
        ...
        const validation = validateInput(BulkCreateHourEntriesSchema, input)
        if (!validation.success) {
            return { error: validation.error }
        }
        ...
        for (const request of approvedRequests) {
            const reqStartDate = startOfDay(new Date(request.startDate))
            const reqEndDate = startOfDay(new Date(request.endDate))
...
export async function batchUpdateHourEntries(input: BatchUpdateHourEntriesInput) {
    try {
        ...
        const validation = validateInput(BatchUpdateHourEntriesSchema, input)
        if (!validation.success) {
            return { error: validation.error }
        }
